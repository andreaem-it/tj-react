...
- [x] Moderazione pre-pubblicazione — `contentModeration.ts`: check deterministici per lunghezza minima, placeholder, markdown rotto, URL sospetti, menzione fonte e mismatch lessicale conservativo fra titolo e contenuto. La verifica asincrona degli URL generati è collegata sia all’elaborazione manuale sia all’autoposter; destinazioni non sicure bloccano, errori HTTP e timeout abbassano il quality score e richiedono revisione (`93256da`).
